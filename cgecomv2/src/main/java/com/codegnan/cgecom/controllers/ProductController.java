...
public class ProductController {
	...
	/*
	 * @GetMapping("/products/add") public String showAddProductPage(HttpSession
	 * session) { // Check if the user is logged in and if they are an admin User
	 * loggedInUser = (User) session.getAttribute("loggedInUser"); if (loggedInUser
	 * == null || !"ADMIN".equals(loggedInUser.getRole())) { // If not logged in or
	 * not an admin, redirect to login return "redirect:/login"; }
	 * 
	 * 
	 * // If logged in and admin, show add-product page return "add-product"; // JSP
	 * file for adding a product }
	 */
	 ...
	 * 
	 * @PostMapping("/products/add") public String addNewProduct(@ModelAttribute
	 * Product product) { productService.saveProduct(product); return
	 * "redirect:/products"; // Redirect to the product list after adding }
	 */
	...
	public String updateProduct(@PathVariable int id, @ModelAttribute Product product,@RequestParam("categoryId") int categoryId) {
		try {
